...
const Validate = () => {
  ...
  // Ensure keyboard opens by focusing hidden input shortly after mount
  useEffect(() => {
    const t = setTimeout(() => hiddenInputRef.current?.focus(), 600);
    return () => clearTimeout(t);
  }, []);

  // Track keyboard hide to reset focus state
  useEffect(() => {
  ...
  const focusHidden = () => {
    ...
    if (!input) return;
    // If already focused, do nothing to avoid flicker/double open
