...
const NotificationsTab = () => {
  ...
  const loadNotifications = async () => {
    await request(api.get("/poster/profile/notifications"));
  };
    ...
    return (
      <View className="flex-1 bg-slate-50">
        ...
        <View className="flex-1 justify-center items-center">
          ...
          <Text className="text-slate-600 mt-4 text-lg">Loading notifications...</Text>
        </View>
      </View>
    );
  }
      ...
      <View className="flex-1 bg-slate-50">
        ...
        >
          <View className="justify-center items-center px-6 py-12">
            ...
            <Text className="text-slate-400 text-2xl font-bold mt-6 text-center">
              No notifications yet
            </Text>
            <Text className="text-slate-400 text-base mt-2 text-center">
              When you receive bids or task updates, they'll appear here
            </Text>
            <TouchableOpacity 
              className="bg-blue-500 px-6 py-3 rounded-xl mt-6"
              onPress={loadNotifications}
      ...
      >
        <View className="px-4 space-y-4">
          {notifications.map((notification, index) => {
            const config = getNotificationConfig(notification.type?.toLowerCase());
            return (
              <TouchableOpacity
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-slate-600 text-sm mb-2">
                        <Text className="font-semibold text-slate-800">
                          {notification.seekerName || 'Unknown User'}
                        </Text>
                        {config.subtitle}
                      </Text>
                      {/* Task Title */}
                      <View className="bg-slate-50 rounded-xl px-3 py-2 mb-3 border border-slate-200">
                        <Text className="text-slate-700 font-semibold text-sm">
                          "{notification.taskTitle || 'Unknown Task'}"
                        </Text>
                      </View>
                      {/* Seeker Stats */}
                      <View className="flex-row items-center space-x-4 mb-3">
                        <View className="flex-row items-center">
                          ...
                          <Text className="text-slate-600 text-xs ml-1 font-semibold">
                            {notification.seekerRating || '0'}
                          </Text>
                        </View>
                        <View className="flex-row items-center">
                          <Ionicons name="checkmark-circle" size={14} color="#10b981" />
                          <Text className="text-slate-600 text-xs ml-1 font-semibold">
                            {notification.completedTasks || 0} completed
                          </Text>
                        </View>
                      </View>
                        ...
                        <View className="flex-row justify-between items-center">
                          <View className="flex-row items-center">
                            ...
                            <Text className="text-green-600 font-bold text-sm ml-1">
                              {notification.bidAmount || 'N/A'}
                            </Text>
                            <Text className="text-slate-500 text-xs ml-2">
                              • {notification.timeline || 'No timeline'}
                            </Text>
                          </View>
              ...
              >
                <View className="px-6 pb-6">
                  ...
                  <View className="flex-row items-center mb-6">
                    <LinearGradient
                      colors={getNotificationConfig(selectedNotification.type?.toLowerCase()).gradient}
                      start={{ x: 0, y: 0 }}
                      end={{ x: 1, y: 0 }}
                      className="w-14 h-14 rounded-2xl items-center justify-center mr-4"
                    >
                      <Ionicons
                        name={getNotificationConfig(selectedNotification.type?.toLowerCase()).icon}
                        size={28}
                        color="#ffffff"
                      />
                    </LinearGradient>
                    <View className="flex-1">
                      <Text className="text-2xl font-bold text-slate-800">
                        {getNotificationConfig(selectedNotification.type?.toLowerCase()).title}
                      </Text>
                      <Text className="text-slate-500 text-sm mt-1">
                        {selectedNotification.time || 'Recently'}
                      </Text>
                    </View>
                  </View>
                  {/* Seeker Profile */}
                    ...
                    <View className="flex-row items-center">
                      <View className="w-16 h-16 bg-purple-500 rounded-2xl items-center justify-center mr-4">
                        <Text className="text-white text-xl font-bold">
                          {selectedNotification.seekerName?.charAt(0) || 'U'}
                        </Text>
                      </View>
                      <View className="flex-1">
                        <Text className="text-xl font-bold text-slate-800">
                          {selectedNotification.seekerName || 'Unknown User'}
                        </Text>
                        <View className="flex-row items-center space-x-4 mt-2">
                          <View className="flex-row items-center">
                            <Ionicons name="star" size={16} color="#f59e0b" />
                            <Text className="text-slate-600 text-sm ml-1 font-semibold">
                              {selectedNotification.seekerRating || '0'} rating
                            </Text>
                          </View>
                          <View className="flex-row items-center">
                            <Ionicons name="checkmark-done" size={16} color="#10b981" />
                            <Text className="text-slate-600 text-sm ml-1 font-semibold">
                              {selectedNotification.completedTasks || 0} tasks
                            </Text>
                          </View>
                        </View>
                      </View>
                  ...
                  <View className="bg-white border-2 border-slate-200 rounded-2xl p-4 mb-6">
                    ...
                    <Text className="text-slate-700 text-base leading-6">
                      {selectedNotification.taskTitle || 'Unknown Task'}
                    </Text>
